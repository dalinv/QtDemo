...
namespace libxl
{
    ...
    template <class TCHAR> struct IBookT
    {
        virtual bool XLAPIENTRY load(
            const TCHAR *filename, const TCHAR *tempFile = 0) = 0;
        virtual bool XLAPIENTRY loadSheet(
            const TCHAR *filename, int sheetIndex, const TCHAR *tempFile = 0) = 0;
        virtual bool XLAPIENTRY loadPartially(const TCHAR *filename, int sheetIndex,
            int firstRow, int lastRow, const TCHAR *tempFile = 0) = 0;

        virtual bool XLAPIENTRY save(const TCHAR *filename, bool useTempFile = false) = 0;

        virtual bool XLAPIENTRY loadRaw(const char *data, unsigned size,
            int sheetIndex = -1, int firstRow = -1, int lastRow = -1) = 0;
        virtual bool XLAPIENTRY saveRaw(const char **data, unsigned *size) = 0;

        virtual ISheetT<TCHAR> *XLAPIENTRY addSheet(
            const TCHAR *name, ISheetT<TCHAR> *initSheet = 0) = 0;
        virtual ISheetT<TCHAR> *XLAPIENTRY insertSheet(
            int index, const TCHAR *name, ISheetT<TCHAR> *initSheet = 0) = 0;
        ...
        virtual IFormatT<TCHAR> *XLAPIENTRY addFormat(
            IFormatT<TCHAR> *initFormat = 0) = 0;
        ...
        virtual double XLAPIENTRY datePack(int year, int month, int day, int hour = 0,
            int min = 0, int sec = 0, int msec = 0) = 0;
        virtual bool XLAPIENTRY dateUnpack(double value, int *year, int *month, int *day,
            int *hour = 0, int *min = 0, int *sec = 0, int *msec = 0) = 0;

        virtual Color XLAPIENTRY colorPack(int red, int green, int blue) = 0;
        virtual void XLAPIENTRY colorUnpack(
            Color color, int *red, int *green, int *blue) = 0;
        ...
        virtual PictureType XLAPIENTRY getPicture(
            int index, const char **data, unsigned *size) const = 0;
